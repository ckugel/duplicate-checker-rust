...
impl FileDataMov {
    pub fn new(size: u64, cap: Box<VideoCapture>) -> Self {
        FileDataMov {size, frame_hashes_size: 0, frame_hashes: [0; 5], cap}
    }

    pub fn compute_frame_long(&mut self, mat: Mat) -> u64{
    ...
    pub fn compute_frames_long(&mut self) {
        while &self.frame_hashes_size < &self.frame_hashes.len() && *&self.cap.is_opened().unwrap() {
            ...
            self.frame_hashes[self.frame_hashes_size % self.frame_hashes.len()] = self.compute_frame_long(frame);
